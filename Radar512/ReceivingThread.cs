...
using Radar512.MathLabFuntions;
            ...
            receivingThread_1024 = new Thread((ThreadStart)delegate ()
            {
               // _Osvezavanje = new RenderMenagemenThreadGrafik_VremenskaBaza(100);
                ...
                while (Exit != true)
                {
                    ...
                    var testBroj = BitConverter.ToUInt32(ms.ToArray(), 12);
                    if (pr.Buffer1[0] == null)
                    {
                        if (testBroj % 2 == 0 && x % 2 == 0)
                        {
                            pr.Buffer1[x] = ms.ToArray();
                        }
                        else if (testBroj % 2 == 1 && x % 2 == 1)
                        {
                            pr.Buffer1[x] = ms.ToArray();
                        }
                        else
                    ...
                    {
                        if (testBroj % 2 == 0 && y % 2 == 0)
                        {
                            pr.Buffer2[y] = ms.ToArray();
                        }
                        else if (testBroj % 2 == 1 && y % 2 == 1)
                        {
                            pr.Buffer2[y] = ms.ToArray();
                        }
                        else
                        ...
                        try
                        {
                            if (listaTakova1.Count != 0)
                            {
                                ...
                                else if (listaTakova1.First.Value.Status == TaskStatus.RanToCompletion)
                                {
                                    ...
                                    RecivingThreadNotificationMessage?.Invoke(this, "Obradjujem... broj lose primljenih paketa:"+counterGreske);
        ...
        private static Task<object[]> VratiKonverovatno2048(byte[][] _buff, CancellationToken cancellationToken = default)
        {
            ...
            lock (_buff)
            {
                ...
                return Task.Run<object[]>(() =>
                {
                        ...
                        cancellationToken.ThrowIfCancellationRequested();
                        return new object[] { c, BitConverter.ToUInt16(new byte[] { a[1][2047, 9], a[1][2047, 10] }, 0) * 0.001 };
